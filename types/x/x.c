...
static void	x_fp_is_vp_zero(t_flags flags, int size) // есть точно такая же функция в d.c - вынести (d_fp_is_vp_zero)
{
	if (flags.val_w > size)
	{
		flags.fl_z = 0;
...
int	x(t_flags flags, va_list args)
{
	...
	{
		x_fp_is_vp_zero(flags, size);
		return (1);
	}
	if (flags.fl_w == 0 && flags.fl_p != 0)
		x_only_p(flags, size, chislo);
	if (flags.fl_w != 0 && flags.fl_p == 0)
		x_only_w(flags, size, chislo);
	if (flags.fl_w != 0 && flags.fl_p != 0)
		x_wp(flags, size, chislo);
	return (sum_x);
}